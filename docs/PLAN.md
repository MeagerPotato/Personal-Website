...
- **Exit insurance:** contract tests run against `dist/` (so the same suite validates any replacement shell). Key invariant: **every page is byte-identical outside `<main>` and `[data-page-head]`.**
