...
| The dock prompt ("Orbit FishAI", "Flying to FishAI" with its "Stop", "Leave orbit"): a real button, bottom centre (its offers are out of sight while the star map is open in the strip above a phone's page sheet, where they would sit on the galaxy: `quiet` in `ui/Prompt.ts`; a journey's Stop always shows) | `.dock-prompt` in `src/styles/global.css` |
